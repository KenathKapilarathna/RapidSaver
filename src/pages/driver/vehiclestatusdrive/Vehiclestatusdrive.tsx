...
const driverEditable = [1, 2, 3, 7, 8, 9];
...
export default function TrackingJourney() {
  ...
  const [role, setRole] = useState<string>("driver"); // or "admin" or "customer"
  ...
  useEffect(() => {
    const getRecentBookings = async () => {
      ...
      setIsLoading(true);
      try {
        const data = await getRecentUserBookingsByDriver(user.id);
  ...
  const saveChanges = async () => {
    {
      if (!user) return;
      if (selectedBooking) {
        const allData = await setBookingStatusByDriver(
          user.id,
          selectedBooking.id,
      ...
      {!isLoading && allBookings.length > 0 && (
        <section>
          ...
          >
            ...
            <p>
              Driver:{" "}
              {(selectedBooking && selectedBooking.driver?.name) || "N/A"}
            </p>
            {/* Services */}
            <h2
              style={{
