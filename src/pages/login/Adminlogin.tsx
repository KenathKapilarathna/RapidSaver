...
const AuthLogin = () => {
  ...
  const dashboardPath = isAdmin ? "/admin" : "/driver";
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : `${role.charAt(0).toUpperCase() + role.slice(1)} login failed. Please check your credentials.`;
  ...
  const getHeaderTitle = () => {
    return isAdmin ? "Admin Portal" : "Driver Portal";
  };

  const getHeaderSubtitle = () => {
    return isAdmin 
      ? "Secure access to administration dashboard" 
      : "Secure access to driver dashboard";
  };
