...
import {
  getPastUserBookingsByAdmin,
  setBookingStatusOnPastBookings,
  setDriverOnPastBooking,
...
const AdminPastBooking = () => {
  ...
  const changeDriver = async (bookingId: string, driverId: string) => {
    setIsLoading(true);
    try {
      const data = await setDriverOnPastBooking(bookingId, driverId);
      const formattedData: Booking[] = data.map((item: any) => ({
        id: item.id,
        userId: item.userId || "",
        driverId: item.driverId || "",
  ...
  const updateBookingStatus = async (
    ...
    setIsLoading(true);
    try {
      const data = await setBookingStatusOnPastBookings(id, status);
      const formattedData: Booking[] = data.map((item: any) => ({
        id: item.id,
        userId: item.userId || "",
        driverId: item.driverId || "",
      ...
      render: (_: any, record: Booking) => (
        <>
          <p>
            ...
            >
              {STEPS[
                record.status as keyof typeof STEPS
              ]?.label.toLocaleUpperCase() || record.status}
            </span>
          </p>
          <select
